...
class Pagination
{
    ...
    public function get()
    {
        ...
        for ($page = $limits[0]; $page <= $limits[1]; $page++) {
            ...
            if ($page == $this->current_page) {
                ...
                $links .= '<li><span class="page-numbers current">' . $page . '</span></li>';
            } else {
        ...
        if (!is_null($links)) {
            ...
            if ($this->current_page > 1)
                ...
                $links = $this->generateHtml(1, '<i class="zmdi zmdi-long-arrow-left"></i>', 'next') . $links;  // &lt;
            # Если текущая страница не первая
            if ($this->current_page < $this->amount)
                # Создаём ссылку "На последнюю"
                $links .= $this->generateHtml($this->amount, '<i class="zmdi zmdi-long-arrow-right"></i>', 'next');  // &gt;
        }
        $html .= $links . '</ul>';
        # Возвращаем html
        return $html;
     ...
     * @return
     */
    private function generateHtml($page, $text = null, $end = '')
    {
        ...
        return
            ...
            //'<li><a href="' . $currentURI . $this->index . $page . '" class="page-numbers">' . $text . '</a></li>';
            '<li><a href="' . $currentURI . $this->index . $page . '" class="page-numbers ' . $end . '">' . $text . '</a></li>';
    }
    /**
